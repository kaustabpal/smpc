...
class NGSIMTest(Node):
	def __init__(self):
		...
		self.g_region_cntr =  torch.tensor([50, 0, np.deg2rad(0)], dtype=dtype)
		self.sampler = Goal_Sampler(torch.tensor(self.agent_pose, dtype=dtype), self.g_region_cntr, 15.0, 0, obstacles=np.array(self.obs_list))
		#self.sampler.centers = torch.tensor(np.tile(self.agent_pose, (self.N+1,1)), dtype=dtype)
	...
	def get_lane_cons_overtake(self):
		...
		for i in range(self.num_goals):
			goal_pose = Pose()
			goal_pose.position.x = 80.0#float(goal[i][0])
			goal_pose.position.y = 14.0#float(goal[i][1])
