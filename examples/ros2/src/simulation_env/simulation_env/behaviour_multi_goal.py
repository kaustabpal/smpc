...
class Behaviour(Node):
	def __init__(self):
		...
		self.sampler.left_lane_bound = -2.0
		self.sampler.right_lane_bound = 2.0
		self.sampler.axis = 1
		self.sampler.num_particles = 50
		self.sampler.init_w_cov = 0.05
	...
	def send_request(self):
		...
		for i in range(10):
			...
			obs_list.append([odom.pose.pose.position.x, odom.pose.pose.position.y, odom.pose.pose.orientation.z])
		...
		self.sampler.obstacles = np.array(obs_list)
		goal = PoseArray()
		lane_cons = PoseArray()
	...
	def get_lane_cons_overtake(self):
		...
		for i in range(self.num_goals):
			...
			goal_pose.position.y = np.clip(float(goal[i][1]), -1.0, 1.0)	#lanes[min_lane_val]	#float(goal[i][1])
	...
	def plot(self, twist, path, kkt):
		...
		for i in range(len(self.goal_p.poses)):
			res_obs_x = np.linalg.norm(self.path_x[i*(self.timesteps+1):i*(self.timesteps+1) + (self.timesteps+1)] - self.nearest_obstacles[:, :(self.timesteps+1)], axis=1)
			res_obs_y = np.linalg.norm(self.path_y[i*(self.timesteps+1):i*(self.timesteps+1) + (self.timesteps+1)] - self.nearest_obstacles[:, (self.timesteps+1):], axis=1)
			res_obs = np.vstack((res_obs_x, res_obs_y))
			res_obs = np.linalg.norm(res_obs, axis=0)
			obs_cost.append(1/np.min(res_obs))
			y_dist.append(np.linalg.norm(self.path_y[i*(self.timesteps+1):i*(self.timesteps+1) + (self.timesteps+1)] - 14.0))
			kkt_cost.append(kkt[i])
			cruise_speed.append(np.linalg.norm(self.path_v[i*(self.timesteps+1):i*(self.timesteps+1) + (self.timesteps+1)] - target_cruise_speed))
			angular_vel.append(np.linalg.norm(self.path_w[i*(self.timesteps+1):i*(self.timesteps+1) + (self.timesteps+1)] - 0.0))
		...
		for i in range(len(self.goal_p.poses)):
			if i == index:
				plt.plot(self.path_x[i*(self.timesteps+1):i*(self.timesteps+1) + (self.timesteps+1)], self.path_y[i*(self.timesteps+1):i*(self.timesteps+1) + (self.timesteps+1)], 'y')
				twist.linear.x = self.path_v[i*(self.timesteps+1)+1]
				twist.angular.z = self.path_w[i*(self.timesteps+1)+1]
				goals.append([self.path_x[i*(self.timesteps+1) + self.timesteps], self.path_y[i*(self.timesteps+1) + self.timesteps]])
			else:
				plt.plot(self.path_x[i*(self.timesteps+1):i*(self.timesteps+1) + (self.timesteps+1)], self.path_y[i*(self.timesteps+1):i*(self.timesteps+1) + (self.timesteps+1)], 'pink')
		plt.plot(self.path_x[index*(self.timesteps+1):index*(self.timesteps+1) + (self.timesteps+1)], self.path_y[index*(self.timesteps+1):index*(self.timesteps+1) + (self.timesteps+1)], 'y')
		...
		self.all_goal_costs.append([self.path_x[index*(self.timesteps+1) + self.timesteps], self.path_y[index*(self.timesteps+1) + self.timesteps]])
		...
		#plt.draw()
		#plt.show()
		plt.savefig("../../results/behaviour/"+str(self.loop)+".png")
		plt.pause(0.0001)
