...
class CombinedEnv(Node):
	def __init__(self) -> None:
		...
		self.sampler.right_lane_bound = 10.0
		self.sampler.axis = 1
	...
	def send_request(self):
		...
		# print(obstacles)
		agent = np.array([0.0, self.agent_pose[1], self.agent_pose[2]])
		self.sampler.c_state = torch.tensor(agent, dtype=self.dtype)
	...
	def get_lane_cons_overtake(self):
		...
		for i in range(self.num_goals):
			goal_pose = Pose()
			goal_pose.position.x = float(goal[i][0]) + self.agent_pose[0]
			goal_pose.position.y = lanes[min_lane_val]	#np.clip(float(goal[i][1]), -8.0, 8.0)	#lanes[min_lane_val]	#float(goal[i][1])
			...
			if x > self.radius*np.pi:
				...
				if x>488:
					...
					if x > self.radius*np.pi:
						...
						env_x = -244 + x
						env_y = y
						env_theta = x/self.radius + np.pi/2 + np.pi/2 + obs[2]
	...
	def plot(self, twist, path, kkt):
		...
		for j in range(self.sampler.traj_N.shape[0]):
			...
			env_theta = []
			for p in range((self.timesteps + 1)):
				agent = [self.sampler.traj_N[j,p,0]+self.agent_pose[0], self.sampler.traj_N[j,p,1], 0.0, self.lane_radii[0]+2]
				x_env, y_env, theta_env = self.centerline_to_env(agent)
				env_x.append(x_env)
				env_y.append(y_env)
				env_theta.append(theta_env)
			plt.plot(np.array(env_x), np.array(env_y), 'r', alpha=0.05)
			# plt.plot(self.sampler.traj_N[j,:,0]+self.agent_pose[0], self.sampler.traj_N[j,:,1], alpha=0.05)
		env_x = []
		env_y = []
		env_theta = []
		for p in range((self.timesteps + 1)):
			agent = [self.sampler.top_trajs[0,p,0]+self.agent_pose[0], self.sampler.top_trajs[0,p,1], 0.0, self.lane_radii[0]+2]
			x_env, y_env, theta_env = self.centerline_to_env(agent)
			env_x.append(x_env)
			env_y.append(y_env)
			env_theta.append(theta_env)
		plt.plot(np.array(env_x), np.array(env_y), color='green')
