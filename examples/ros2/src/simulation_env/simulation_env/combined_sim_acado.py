		...
		for i in range(self.num_goals):
			...
			if i == index:
				...
				for p in range((self.timesteps + 1)):
					...
					env_x.append(x_env)
					env_y.append(y_env)
				plt.plot(np.array(env_x), np.array(env_y), 'r', alpha=0.2)
		env_x = []
		env_y = []
		for p in range((self.timesteps + 1)):
			agent = [path_x[index*(self.timesteps + 1) + p], path_y[index*(self.timesteps + 1) + p], path_theta[index*(self.timesteps + 1) + p], self.lane_radii[0]+2]
			x_env, y_env, theta_env = self.centerline_to_env(agent)
			env_x.append(x_env)
			env_y.append(y_env)
		env_theta = np.arctan2(np.diff(np.array(env_y)), np.diff(np.array(env_x)))
		plt.plot(np.array(env_x), np.array(env_y), color='green')
		fv = path_v[index*(self.timesteps + 1)+1]
